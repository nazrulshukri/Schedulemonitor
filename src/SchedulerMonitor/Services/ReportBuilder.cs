...
public sealed class ReportBuilder
{
    ...
    private static string BuildHtml(MonitorRunResult run, IReadOnlyList<TaskMonitorResult> ordered)
    {
        var attention = ordered.Where(task => task.Status is MonitorStatus.Failed or MonitorStatus.Overdue
            or MonitorStatus.Disabled or MonitorStatus.Unreachable).ToList();
        ...
        sb.Append("<!doctype html><html><head><meta charset='utf-8'><style>")
          ...
          .Append(Card("Pending", Count(run, MonitorStatus.Pending))).Append(Card("Problems", run.Tasks.Count(task => task.Status is MonitorStatus.Failed or MonitorStatus.Overdue or MonitorStatus.Disabled or MonitorStatus.Unreachable)))
          .Append("</div>");
        ...
        sb.Append("<h2>All Tasks</h2>").Append(BuildTable(ordered))
          .Append("<div class='foot'>Generated by Scheduler Monitor. Windows result codes are retained for troubleshooting.</div>")
    ...
    {
        var sb = new StringBuilder("<table><thead><tr><th>Server</th><th>Task</th><th>Status</th><th>Last Run</th><th>Result</th><th>Detail</th></tr></thead><tbody>");
        foreach (var task in tasks)
        {
            sb.Append("<tr>")
              .Append(Td(task.ServerName)).Append(Td(task.DisplayName))
              .Append($"<td class='status {task.StatusText}'>{task.StatusText}</td>")
    ...
    private static int SortOrder(MonitorStatus status) => status switch
    {
        ...
        MonitorStatus.Disabled => 3, MonitorStatus.Running => 4, MonitorStatus.Pending => 5, _ => 6
    };
}
