...
--                   rolled back with it.
--
-- Both cover AWACSWSTYPE, AWACSRECIPEBYWSTYPE, AWACSLF, TBLWIREBOND and
-- ENGINEERING. The snapshot is the whole row as JSON, read from the data
-- dictionary at run time, so adding a column to one of those MES tables
