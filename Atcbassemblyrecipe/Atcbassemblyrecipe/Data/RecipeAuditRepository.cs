...
namespace Atcbassemblyrecipe.Data
{
    ...
    internal sealed record AuditedTable(string Name, string KeyPredicate)
    {
        ...
        // ENGINEERING is addressed by ROWID for the same reason as the three
        // above: its TBLROWID is filled on insert for the MES side, but rows
        // carried over from the old SQL Server table have none.
        private static readonly AuditedTable Engineering =
            new(AuditedTableNames.Engineering, "ROWID = CHARTOROWID(:row_key)");
        ...
        public static AuditedTable For(string tableName)
        {
            return tableName.Trim().ToUpperInvariant() switch
            {
                ...
                AuditedTableNames.Engineering => Engineering,
