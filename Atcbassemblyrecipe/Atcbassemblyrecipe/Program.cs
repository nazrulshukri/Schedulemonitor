...
// The ENGINEERING column mapping is per-user (it depends on the caller's module
// grants) and cached across requests in the shared memory cache, so the provider
// is scoped like the evaluator it asks.
builder.Services.AddScoped<IEngineeringColumnGroupProvider, EngineeringColumnGroupProvider>();
